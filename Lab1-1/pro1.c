...
double waste_time(long n)
{
	...
	CPU_ZERO(&cpuset);
	CPU_SET(1,&cpuset);
	printf("Change Core 1!\n");
        if (pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset) !=0) 	    {perror("pthread_setaffinity_np");}
	}
	if(i==500000000)
	{
	CPU_ZERO(&cpuset);
	CPU_SET(2,&cpuset);
	printf("Change Core 2!\n");
        if (pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &cpuset) !=0) 	    {perror("pthread_setaffinity_np");}
	}	
        if(i==800000000)
	{
